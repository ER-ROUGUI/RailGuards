...
class Train2SensorPublisher(Node):
    def __init__(self):
        super().__init__('train2_sensor_publisher')

        train2 = 'train2'
        # Analog sensor publishers
        self.tp2_pub = self.create_publisher(Float32, f'{train2}/TP2', 10)
        self.tp3_pub = self.create_publisher(Float32, f'{train2}/TP3', 10)
        self.h1_pub = self.create_publisher(Float32, f'{train2}/H1', 10)
        self.dv_pressure_pub = self.create_publisher(Float32, f'{train2}/DV_pressure', 10)
        self.reservoirs_pub = self.create_publisher(Float32, f'{train2}/Reservoirs', 10)
        self.oil_temperature_pub = self.create_publisher(Float32, f'{train2}/Oil_temperature', 10)
        self.motor_current_pub = self.create_publisher(Float32, f'{train2}/Motor_current', 10)

        # Digital sensor publishers
        self.comp_pub = self.create_publisher(Int32, f'{train2}/COMP', 10)
        self.dv_eletric_pub = self.create_publisher(Int32, f'{train2}/DV_eletric', 10)
        self.towers_pub = self.create_publisher(Int32, f'{train2}/Towers', 10)
        self.mpg_pub = self.create_publisher(Int32, f'{train2}/MPG', 10)
        self.lps_pub = self.create_publisher(Int32, f'{train2}/LPS', 10)
        self.pressure_switch_pub = self.create_publisher(Int32, f'{train2}/Pressure_switch', 10)
        self.oil_level_pub = self.create_publisher(Int32, f'{train2}/Oil_level', 10)
        self.caudal_impulses_pub = self.create_publisher(Int32, f'{train2}/Caudal_impulses', 10)
    ...
    def publish_data(self):
        ...
        self.get_logger().info("Train 2: Published sensor data.")


def main(args=None):
    rclpy.init(args=args)
    node = Train2SensorPublisher()
    rclpy.spin(node)
    rclpy.shutdown()
